...
### Save your Smartabase credentials across R sessions {#save_crednentials}
...
2. Trigger a masked password pop-up input. There are two ways to do this:
    1. Manually set `password = "prompt"` within `pull_smartabase()` / `push_smartabase()`
    2. Do not record password anywhere, including in .Renviron file via `save_credentials()`
    3. Manually type all credentials within each in `pull_smartabase()` / `push_smartabase()` call. You could supply some of the credentials in the API calls, and save others using `save_credentials()`.
...
**You need to make sure you never share the .Renviron file and understand the risks of not encrypting your Smartabse login credentials**. 
